...
- [JDK9](#jdk9)
  ...
  - [G1 成为默认垃圾回收器（GC）](#g1-成为默认垃圾回收器gc)
    - [主要特性](#主要特性)
    - [代码体验](#代码体验)
    - [手动指定使用其它 GC](#手动指定使用其它-gc)
  - [JShell（交互式 REPL 工具）](#jshell交互式-repl-工具)
  - [其他新特性](#其他新特性)
    - [私有接口方法](#私有接口方法)
    - [集合工厂方法](#集合工厂方法)
    - [try-with-resources 改进](#try-with-resources-改进)
    - [Stream API 增强](#stream-api-增强)
- [JDK11](#jdk11)
    - [新特性介绍](#新特性介绍)
      - [1. 新的字符串方法](#1-新的字符串方法)
      - [2. 本地变量类型推断支持 lambda 参数（升级）](#2-本地变量类型推断支持-lambda-参数升级)
      - [3. 文件 I/O 增强](#3-文件-io-增强)
        - [a) `Files.readString` 和 `Files.writeString`](#a-filesreadstring-和-fileswritestring)
        - [b) `InputStream.transferTo(OutputStream)`](#b-inputstreamtransfertooutputstream)
      - [4. 集合工厂方法增强](#4-集合工厂方法增强)
      - [5. 新的 Http Client API 标准](#5-新的-http-client-api-标准)
      - [6. 垃圾回收器变化](#6-垃圾回收器变化)
      - [7. 新的类库和API移除](#7-新的类库和api移除)
      - [8. 其他](#8-其他)
  - [局部类型推断Var](#局部类型推断var)
    - [1.初始化定义局部变量](#1初始化定义局部变量)
    - [2.增强for循环](#2增强for循环)
    - [3.传统for循环的index定义](#3传统for循环的index定义)
    - [4.try包裹的变量](#4try包裹的变量)
    - [5.lambda中隐含的var推断](#5lambda中隐含的var推断)
  - [字符串API增强](#字符串api增强)
...
- [JDK21](#jdk21)
  ...
  - [Switch类型匹配](#switch类型匹配)
    - [基本语法：](#基本语法)
    - [结合守卫（when）进一步增强：](#结合守卫when进一步增强)
    - [使用示例：](#使用示例)
  - [虚拟线程](#虚拟线程)
    - [主要特性](#主要特性-1)
    - [基本用法示例](#基本用法示例)
    - [使用 Executors 新 API 批量提交任务](#使用-executors-新-api-批量提交任务)
    - [主要意义](#主要意义)
  - [分代ZGC](#分代zgc)
    - [JDK21 引入的分代ZGC（Generational ZGC）](#jdk21-引入的分代zgcgenerational-zgc)
      - [主要特性](#主要特性-2)
      - [启用分代ZGC](#启用分代zgc)
      - [代码体验（无需代码改动，参数可用即生效）](#代码体验无需代码改动参数可用即生效)
      - [小结](#小结)
- [JDK22](#jdk22)
  - [未命名变量](#未命名变量)
<!-- /TOC -->
