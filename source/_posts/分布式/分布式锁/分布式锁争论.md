...
2. 崩溃恢复与数据持久化
这是一个非常关键的安全隐患。文档描述了以下场景：

无持久化（No Persistence）：客户端 A 在 5 个节点中的 3 个上加锁成功。随后，其中一个节点因故障重启。由于没有持久化，该节点丢失了所有数据。此时，这个刚重启的节点（原本存有 A 的锁）又可以接受新锁了。于是，客户端 B 可以再次向包括这个重启节点在内的 3 个节点加锁成功。这样就同时有两个客户端持有锁，违反了互斥性。

为了解决这个问题，文档给出了两种方案，但这两种方案都意味着需要在性能和安全性之间做出取舍：

1. 方案一：fsync=always	每次写入都强制刷盘，确保节点重启后数据不丢失。	性能极低，写入性能会大幅下降。适用对数据安全性要求极高，能接受性能损耗的场景。
2. 方案二：延迟重启（Delayed Restarts）	节点崩溃后，不立即重启，而是等待一个比最大 TTL 更长的时间（例如 30 秒以上）再重启。性能极高，但节点在重启前长时间不可用，影响集群整体容量。适用可以容忍节点长时间不可用，且对性能要求高的场景。

延迟重启的原理：节点离线时间超过 TTL 后，它之前持有的所有锁都已经自动过期。此时重启，它是一张白纸，不会包含任何有效的锁信息，因此不可能被新客户端利用来破坏互斥性。
...
## 活性保证（Liveness Guarantee）

RedLock 通过以下三种机制保证系统的活性（即系统不会进入永久死锁状态）：

1. **自动租约过期**：所有锁键均设置 TTL。即使客户端崩溃或网络分区，锁也会在 TTL 后自动失效，系统可恢复。

2. **主动解锁**：客户端在加锁失败或任务完成后，主动发起解锁操作。这加速锁释放，无需等待 TTL 到期。

3. **重试退避策略**：客户端重试时采用指数退避或随机延迟，减少多个客户端在发生网络分区时的冲突概率。

## 可用性代价

然而，文献中也明确指出了代价：

在网络分区期间，系统的可用性会受到 TTL 时间的惩罚。若客户端持有锁后发生分区，其他客户端需等待 TTL 到期才能重新获取锁。如果分区持续，系统可用性可能被无限延伸。
...
若业务执行时间较长，客户端可在锁快要过期时使用原子 Lua 脚本尝试延长 TTL，但**必须同样向多数派节点发起续期**。

注意：续期次数必须受限，否则永不释放的锁会违反活性原则。
